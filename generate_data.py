...
Sport adaptation vs MESSI:
  - confederations are display-only regions: Europe / North America / Asia /
    Oceania / Africa
  - margins are "goals" (hockey), no draws -- W-L records only
  ...
  - all four events appear under one Global pill group (no continental
    championships in international hockey); the continental-winner gold-pill
...
# Tournaments that crown a champion (all four international hockey events).
MEDAL_TOURNAMENTS = ["Olympics", "World Cup of Hockey", "Canada Cup",
                     "IIHF World Championship"]

# Flagship tournaments anchor GOAT entries. 👑 = Olympics (hockey's true
# crown; best-on-best). World Cup of Hockey + Canada Cup are also pinnacle
# best-on-best events. IIHF Worlds still feeds the rolling rating but does NOT
# anchor a GOAT entry (it's annual + held during NHL playoffs).
FLAGSHIP_TOURNAMENTS = ["Olympics", "World Cup of Hockey", "Canada Cup"]

# Tournament selector grouping for the Tournaments tab pills. International
# hockey has no continental championships, so all four events are Global.
GLOBAL_TOURNAMENTS = ["Olympics", "World Cup of Hockey", "Canada Cup",
                      "IIHF World Championship"]
...
TOURNAMENT_LABELS = {
    ...
    "World Cup of Hockey":     ("WCoH Final",     2),
    "Canada Cup":              ("Canada Cup Final", 3),
    "IIHF World Championship": ("IIHF WC Final",  4),
}

# Tournament name -> short label for honor badges.
TOURNAMENT_ABBREV = {
    "Olympics":                "Oly",
    "World Cup of Hockey":     "WCoH",
    "Canada Cup":              "CC",
    "IIHF World Championship": "IIHF",
}

# Country -> ISO alpha-2 for flag emoji. Defunct entities (Soviet Union,
# Czechoslovakia, East/West Germany, Yugoslavia, Unified Team) have no flag
# emoji and are left out -- the UI shows the name without a flag.
COUNTRY_TO_ISO = {
    # North America
    "United States": "US", "Canada": "CA", "Mexico": "MX",
    # Europe (primary hockey block)
    "Russia": "RU", "Sweden": "SE", "Finland": "FI",
    "Czech Republic": "CZ", "Slovakia": "SK", "Germany": "DE",
    "Switzerland": "CH", "Norway": "NO", "Denmark": "DK",
    "Austria": "AT", "Latvia": "LV", "Belarus": "BY", "Kazakhstan": "KZ",
    "France": "FR", "Italy": "IT", "Great Britain": "GB",
    "Netherlands": "NL", "Poland": "PL", "Hungary": "HU",
    "Romania": "RO", "Ukraine": "UA", "Slovenia": "SI", "Croatia": "HR",
    "Serbia": "RS", "Bulgaria": "BG", "Estonia": "EE", "Lithuania": "LT",
    "Belgium": "BE", "Spain": "ES", "Iceland": "IS",
    "Greece": "GR", "Turkey": "TR", "Ireland": "IE", "Luxembourg": "LU",
    "Armenia": "AM", "Bosnia and Herzegovina": "BA",
    # Asia
    "Japan": "JP", "South Korea": "KR", "China": "CN", "Israel": "IL",
    "North Korea": "KP", "Mongolia": "MN", "Hong Kong": "HK",
    # Oceania
...
# Confederation lookup is keyed by COUNTRY NAME in this file (matching
# the ratings `country` column), built by inverting forsberg's code-keyed
# CONFEDERATION via CODE_TO_COUNTRY.
CONFEDERATION = {CODE_TO_COUNTRY[c]: _CONF_BY_CODE[c]
                 for c in CODE_TO_COUNTRY if c in _CONF_BY_CODE}
...
# ============================================================
print("Reading data...")
df = pd.read_csv("forsberg_ratings.csv")
# Align FORSBERG's schema with ICHIRO's so the downstream pipeline (cloned from
# generate_data.py for ICHIRO) reads the same column names: country->name,
# date->ranking_date, games_in_window->games_played.
...
df["is_game_day"] = 1  # every forsberg snapshot date is a game-day

games = pd.read_csv("all_games.csv")
games["date"] = pd.to_datetime(games["date"]).dt.date
# Apply forsberg's code canonicalization (SVN->SLO, "RUSSIA"->RUS, etc.) before
# mapping to display country names -- so a team's history isn't split by alias.
games["home_team"] = games["home_team"].replace(CANON_CODE).map(code_to_country).map(canon_name)
games["road_team"] = games["road_team"].replace(CANON_CODE).map(code_to_country).map(canon_name)
...
# Podium teams are stored as IOC codes; map to display country names.
podiums["team"] = podiums["team"].replace(CANON_CODE).map(code_to_country).map(canon_name)
...
# ── Per-team W-L within a single tournament edition (no draws in hockey) ────
...
# as the "Final", which is wrong (the 2026 IIHF WC mid-QFs got tagged "IIHF
# WC Final" before this gate). Signals accepted:
#   1. A game with round=='final' or round=='bronze' (medal games played ->
#      tournament concluded; bronze covers IIHF WCs whose scraper didn't tag
#      'final').
...
# Olympics > IIHF Worlds > best-on-best invitationals > last game-day of the year.
ANCHOR_PRIORITY = [
    ("Olympics",                "End of Olympic hockey"),
    ("IIHF World Championship", "End of IIHF World Championship"),
    ("World Cup of Hockey",     "End of World Cup of Hockey"),
    ("Canada Cup",              "End of Canada Cup"),
]
